...
export default function NewProduct() {
  ...
  const [specs, setSpecs] = useState<Spec[]>([{ key: '', value: '', modalContent: '' }]);
  const [keyElements, setKeyElements] = useState<KeyElement[]>([{ tag: 'Feature 01', title: '', description: '', images: [] }]);
  ...
  function addSpec() { setSpecs(prev => [...prev, { key: '', value: '', modalContent: '' }]); }
  ...
  function removeKE(i: number) { setKeyElements(prev => prev.filter((_, idx) => idx !== i)); }
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: 60 }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>Product Created</div>
        <div style={{ fontSize: 13, color: '#888', marginBottom: 32 }}>"{form.title}" has been added to the store.</div>
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
          <a href="/admin/products/new" style={btnStyle('#111', '#fff')}>Add Another</a>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="fc">
              ...
              <div className="fgrp">
                ...
                <div className="fh">Auto-generated from title</div>
              </div>
              <div className="fgrp">
              ...
              <div className="fr2">
                ...
                <div className="fgrp">
                  ...
                  <input className="fi" value={form.category} onChange={e => setField('category', e.target.value)} placeholder="e.g. Bag" />
                </div>
              </div>
            </div>
            ...
            <div className="fc">
              ...
              <div className="fgrp">
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <input className="fi" value={form.coverImage} onChange={e => setField('coverImage', e.target.value)} placeholder="Paste URL or upload below" />
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                    {form.images.map((img, i) => (
                      <div key={i} style={{ position: 'relative' }}>
                        ...
                        <button type="button" onClick={() => removeGalleryImage(i)} style={{ position: 'absolute', top: 2, right: 2, background: '#c0392b', color: '#fff', border: 'none', width: 18, height: 18, fontSize: 10, cursor: 'pointer', lineHeight: 1 }}>✕</button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="fc">
              ...
              {specs.map((spec, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr auto', gap: 8, marginBottom: 10 }}>
                  <input className="fi" placeholder="Key (e.g. MAKER)" value={spec.key} onChange={e => setSpec(i, 'key', e.target.value)} />
                  <input className="fi" placeholder="Value" value={spec.value} onChange={e => setSpec(i, 'value', e.target.value)} />
                  <button type="button" onClick={() => removeSpec(i)} style={{ background: '#f5ede6', border: 'none', color: '#c0392b', cursor: 'pointer', padding: '0 10px', fontSize: 14 }}>✕</button>
                  <div style={{ gridColumn: '1/-1' }}>
                    <input className="fi" placeholder="Modal content (HTML allowed)" value={spec.modalContent} onChange={e => setSpec(i, 'modalContent', e.target.value)} />
                  </div>
                </div>
              ))}
                ...
                <div key={i} style={{ border: '1px solid #e8e6de', padding: 16, marginBottom: 16, position: 'relative' }}>
                  ...
                  <div className="fr2" style={{ marginBottom: 10 }}>
                    <div>
                      ...
                      <input className="fi" placeholder="Feature 01" value={ke.tag} onChange={e => setKE(i, 'tag', e.target.value)} />
                    </div>
                    <div>
                      <label className="fl">Title</label>
                      <input className="fi" placeholder="Feature title" value={ke.title} onChange={e => setKE(i, 'title', e.target.value)} />
                    </div>
                  </div>
                  <div className="fgrp">
                    <label className="fl">Description</label>
                    <textarea className="fta" rows={2} placeholder="Feature description" value={ke.description} onChange={e => setKE(i, 'description', e.target.value)} />
                  </div>
                  <div className="fgrp">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="fc">
              ...
              <div style={{ marginTop: 20 }}>
                <button type="submit" disabled={saving} style={{ ...btnStyle('#111', '#fff'), width: '100%', padding: 13 }}>
